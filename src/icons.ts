/**
 * Custom icon sets. Each top-level key becomes an icon *prefix* (e.g. `ru`);
 * each icon is then referenced as `<prefix>:<name>` (e.g. `ru:matter`)
 * anywhere Home Assistant accepts an icon.
 *
 * To add an icon: grab the SVG `d` attribute (path data) of a 24x24 icon and
 * add an entry under the desired set. `viewBox` is optional and defaults to
 * "0 0 24 24".
 *
 * Registration uses the documented custom-iconset API:
...
const ICON_SETS: Record<string, Record<string, IconDefinition>> = {
  ru: {
    // Matter brand logo (source: assets/matter.svg, 24x24). Home Assistant
    // renders this monochrome — the original #44739e fill is dropped in favor
    // of the theme's currentColor, which is standard for <ha-icon>.
    matter: {
    ...
    // Zigbee brand logo (source: assets/zigbee.svg, 24x24; svgrepo.com/svg/305631).
    zigbee: {
    ...
    // Philips Hue brand logo (source: assets/philips-hue.svg, 24x24; svgrepo.com).
    "philips-hue": {
    ...
    // Shelly brand logo (source: assets/shelly.svg, 24x24).
    shelly: {
    ...
    // Ubiquiti UniFi brand logo (source: assets/ubiquiti-unifi.svg, 512x512 —
    // needs the viewBox override).
    "ubiquiti-unifi": {
...
export function registerIconSets(): void {
  ...
  for (const [setName, icons] of Object.entries(ICON_SETS)) {
    window.customIconsets[setName] = async (iconName) => {
      ...
      if (!icon) {
        throw new Error(
          `[hass-custom] icon "${setName}:${iconName}" is not defined`
        );
      }
...
export function iconSetNames(): string[] {
